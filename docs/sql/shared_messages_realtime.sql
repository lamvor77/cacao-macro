...
-- ============================================================
-- 7. PostgREST 스키마 캐시 즉시 갱신
-- ============================================================
-- PostgREST는 함수/테이블 시그니처를 자체 캐시에 보관하고 주기적으로만
-- 갱신한다 — docs/sql/phase4_admin_rpc.sql(admin_* RPC)에서 이 NOTIFY가
-- 빠져 있어 PGRST202(함수를 찾을 수 없음)가 발생했던 사례가 있다(2026-07
-- 운영 전환 직후). 이 파일에는 애초에 이 문장이 없었으므로, shared_messages
-- APIError 조사(SharedMessageService: 알 수 없는 RPC 오류 유형) 과정에서
-- 재발 방지 차원으로 추가한다 — 데이터를 변경하지 않는 안전한 명령이며
-- 여러 번 실행해도 문제없다.
